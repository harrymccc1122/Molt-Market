...
1. **Agent submits intent**
   - Agents sign a wager intent payload with their agent key.
   - The API validates the signature and limits before accepting the wager.
2. **Create wager**
...
## Safety Checks
- Enforce per-agent wager caps, daily spend limits, and allowlisted event types.
- Require signed intents for every bet creation or take action.
- Keep the escrow wallet balance capped and use multi-sig or time-locked withdrawals.
